...
def duckdb_table_metadata(con, include_paritioned_data=True):
    ...
    tablesDF = con.execute(
        "select table_name from information_schema.tables where table_name not like '%_v1' and table_name {in_clause} ( select table_name from information_schema.columns WHERE column_name = 'dayPK' ) order by all"
    ).df()
