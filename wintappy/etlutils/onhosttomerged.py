        ...
        if os.path.isdir(src_path):
            for src_filename in os.listdir(src_path):
                if src_filename.endswith(".parquet"):
                    ...
                    (hostname, event_type, data_capture_epoch) = parse_filename(src_filename)
                    ...
                    dst_filename=f"{hostname}+raw_{event_type}+{data_capture_epoch}.parquet"
                    if not os.path.exists(f"{local_file_path}/{dst_filename}"):
                        #print(f"Exists, skipping: {dst_filename}")
#                    else:
                        os.makedirs(local_file_path, exist_ok=True)
                        shutil.copy2(f"{src_path}/{src_filename}", f"{local_file_path}/{dst_filename}")
